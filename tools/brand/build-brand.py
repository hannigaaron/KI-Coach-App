...
def parts(color, sweep=SWEEP, show_track=True):
...
def wordmark(color, sweep=SWEEP, show_track=True):
    body, x_off = parts(color, sweep, show_track)
    return (f'<g>{body}</g>\n'
            f'  <g transform="translate({x_off:.2f} 0)"><path fill="{color}" d="{REST["d"]}"/></g>')

def lockup(color, claim_color, sweep=SWEEP, show_track=True):
...
files = {
    ...
    "daevo-lockup-ring-offen.svg": svg(LOCKUP_VB, 320, lockup(BRAND, INK, sweep=270), LABEL),
